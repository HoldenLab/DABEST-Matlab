function  ss=FscatJit2_med(identifiers, data, varargin)
...
% Sameer Aryal Jan 22, 2013. Can now compute median difference and plot it
...
%% Insert median difference and CIs on a different axis if there is a pair
if length(celld)==2;
    ...
    avr=repmat(ss.median,2, 1);
    moes=abs(avr-ss.medianCi);
    delta_name = {[uidents{2}, ' minus ', uidents{1}]}; 
    stats_delta = table(delta_name, ss.median, ss.medianCi', NaN, 'VariableNames', {'Group','Value','CIs','N'});
    ...
    if isempty(lims) == 0
        ...
        [num, num3, ax1Pos, x, y, x2, y2, yNew, ~] = setThirdAxis(refAxes, av, ss.median);
        if ss.median < 0
    ...
    elseif ss.median > 0
        ...
        while (num3*y2-(num3-1)*y) >= ax1Pos(2) + ax1Pos(4);
            ...
            newRefLims = [refLims(1)-refDiff refLims(2)+refDiff];
            set(refAxes, 'YLim', newRefLims);
            [num, num3, ax1Pos, x, y, x2, y2, ~, ~] = setThirdAxis(refAxes, av, ss.median);
        end
    ...
    if strcmp(isPaired, 'Y')
        ...
        % Plot the lines that join the medians to the difference axis.
